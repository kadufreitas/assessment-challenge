...
app.get('/me', async (req, res) => {
  try {
    // Test the new token by making an authenticated call to the API.
    const userData = await getUserInfo(req?.headers?.authorization)
...
function composeErrorPage(data, state) {
  ...
  if (data.state && data.state !== state) {
    ...
    content +=
      '<p>Please confirm that you followed the steps in the README, and check the console log.</p>'
  }
